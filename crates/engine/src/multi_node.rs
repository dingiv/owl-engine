...
pub fn run_worker_daemon(
    econfig: &crate::config::EngineConfig,
...
pub fn run_worker_daemon(
    _econfig: &crate::config::EngineConfig,
    // T3 回填:wire dtype 走 owl_nn::Dtype(nccl 多卡路径另行设计,A2.5)
    _dtype: owl_nn::Dtype,
) -> crate::Result<()> {
    crate::bail!("Multi-node inference requires the `nccl` feature to be enabled");
}
