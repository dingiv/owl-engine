...
use crate::env::{mamba_snapshot_block_stride_blocks, MAMBA_SNAPSHOT_BLOCK_STRIDE_ENV};
...
impl BlockManager {
    ...
    fn allocate_fresh(&mut self, seq: &mut Sequence) -> Result<()> {
        ...
        for _ in 0..seq.num_blocks() {
            let block_id = self
                .free_block_ids
                .pop_front()
                .ok_or_else(|| crate::Error::Msg("No free blocks available, retry later!".into()))?;
    ...
    pub fn ensure_allocate(&mut self, seq: &mut Sequence) -> Result<()> {
        ...
        for i in seq.block_table.len()..seq.num_blocks() {
            let Some(block_id) = self.free_block_ids.pop_front() else {
                ...
                crate::bail!("No free blocks available, retry later!");
            };
            self.allocate_block(block_id);
            seq.block_table.push(block_id as u32);
    ...
    pub fn swap_out(&mut self, seq: &mut Sequence) -> Result<()> {
        ...
        if self.free_cpu_block_ids.len() < num_blocks {
            crate::bail!("Not enough CPU swap blocks for seq {}", seq.id);
        }
        ...
        for &gpu_bid_u32 in &seq.block_table {
            ...
            let cpu_bid = self
                .free_cpu_block_ids
                .pop_front()
                .ok_or_else(|| crate::Error::Msg("No free CPU swap blocks".into()))?;
    ...
    fn swap_out_suffix(&mut self, seq: &mut Sequence) -> Result<()> {
        let (prefix_blocks, suffix_blocks) = self.split_prefix_suffix(seq);
        if suffix_blocks == 0 {
            crate::bail!("Seq {} has no sequence-owned suffix blocks to swap", seq.id);
        }
        if self.free_cpu_block_ids.len() < suffix_blocks {
            crate::bail!("Not enough CPU swap blocks for seq {} suffix", seq.id);
        }
        ...
        {
            crate::bail!("Seq {} suffix contains shared KV blocks", seq.id);
        }
    ...
    pub fn swap_in(&mut self, seq: &mut Sequence) -> Result<()> {
        ...
        let cpu_ids = self
            .swapped_map
            .remove(&seq.id)
            .ok_or_else(|| crate::Error::Msg("No CPU-swap entry for seq".into()))?;
        ...
        if cpu_ids.len() > seq.block_table.len() {
            ...
            self.swapped_map.insert(seq.id, cpu_ids);
            crate::bail!("Insufficient GPU blocks to swap in sequence {}", seq.id);
        }
    ...
    fn swap_in_suffix(&mut self, seq: &mut Sequence) -> Result<()> {
        let state = self
            ...
            .get(&seq.id)
            .cloned()
            .ok_or_else(|| crate::Error::Msg("No partial CPU-swap entry for seq".into()))?;
