...
/// 失效 = 结构化报错而非 Xid 盲死。
/// Send 豁免:CUgraph/CUgraphExec 是 context 亲和句柄(非线程绑定的
/// 野指针);owl 契约 = 图只在 actor 线程(A3 每卡一线程)launch,
/// 跨线程仅是"所有权转移"(asyncrt 命令面),使用仍单线程。
unsafe impl Send for DeviceGraph {}
