...
// nvrtc 裁剪:FP8 分支删除(marlin-ffi 归属)/<stdexcept><algorithm> 删除/
// std::is_same → constexpr false(scalar_t == cache_t 单档实例化)/
// fast_tanh host 分支死代码保留(_nvrtc 下 __CUDA_ARCH__ 生效被预处理掉)
/*
...
// owl:nvrtc 无 <type_traits>,自带 bf16 判别(实例化档 scalar==cache 用)
namespace vllm {
template <typename T> struct OwlIsBf16 { static constexpr bool value = false; };
template <> struct OwlIsBf16<__nv_bfloat16> { static constexpr bool value = true; };
} // namespace vllm
...
inline __device__ float fast_tanh(float x) {
    ...
    #endif
  #else
  return tanhf(x); // owl:nvrtc 无 <cmath>,host 分支反正被预处理掉
  #endif
}
...
__device__ void paged_attention_kernel(
  ...
  constexpr bool is_quantized = false; // owl:单档实例化 scalar==cache
...
__device__ void paged_attention_v1_kernel(
  ...
  const float softscapping,
  const int sliding_window) {
  if constexpr (OwlIsBf16<scalar_t>::value) {
...
template<
  ...
  int NUM_THREADS,
  int PARTITION_SIZE>
__device__ void paged_attention_v2_kernel(
  float* __restrict__ exp_sums,           // [num_seqs, num_heads, max_num_partitions]
...
template<
  ...
  int NUM_THREADS,
  int PARTITION_SIZE>
__device__ void paged_attention_v2_reduce_kernel(
  scalar_t* __restrict__ out,             // [num_seqs, num_heads, head_size]
